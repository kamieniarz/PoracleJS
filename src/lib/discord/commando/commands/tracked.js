	...
	try {
		...
		if (!client.config.general.disableQuest) {
			...
			quests.forEach((quest) => {
				...
				message = message.concat(`\n${client.translator.translate('reward').charAt(0).toUpperCase() + client.translator.translate('level').slice(1)}: ${rewardThing} | ${client.translator.translate('distance')}: ${quest.distance}m `)
			})
		}
		...
		if (!client.config.general.disablePokestop) {
			...
			invasions.forEach((invasion) => {
				...
				message = message.concat(`\n${client.translator.translate('invasion').charAt(0).toUpperCase() + client.translator.translate('level').slice(1)}: ${client.translator.translate('Grunt type')}: ${typeText} | ${client.translator.translate('Gender')}: ${genderText}`)
			})
		}
